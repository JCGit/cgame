...
local _writePath = cc.FileUtils:getInstance():getWritablePath()
local _updPath = _writePath .. 'upd/'

local _option = {

	URL_VER = UPDINFO.VERLIST_URL,
	CUR_VER = _writePath .. 'version.json',
}

local upder = class("upder")
...
function upder:downloadZip()
	...
	local verzipCB = function(tag, ...)
		...
		elseif tag == 'fileSuccess' then
			...
			stateinfo.errcode = UPDINFO.CODE.ERROR_OK
			ZipUtils:uncompressZip(params[3], _updPath)	--主线程操作
	...
	if not self:isFileExist(_updPath) then
		lfs.mkdir(_updPath)
	end
	...
	for i=1,#ziplist do
		...
		curTag = info.tag
		self.dlder.downloadFile(info.addr, _updPath, verzipCB)
		self.suspendUpdate()
		if not ret then
...
function upder:localVersion()
	...
	if not self:isFileExist(_option.CUR_VER) then
		return CONFIG_NET.VERSION
	else
		local jsonstr = cc.FileUtils:getInstance():getStringFromFile(_option.CUR_VER)
		local vljson = json.decode(jsonstr,1)
...
function upder:writeLocalVersion(ver, tag)
	...
	io.writefile(_option.CUR_VER, content, 'w')
end
